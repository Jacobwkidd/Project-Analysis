        ...
        if((await User.findAll()).length < TOT_NUM_USERS){
		        ...
		        TODO: Write your seed logic here. Note that you will likely
					...
					*/
                            ...
                            const user2 = await User.create({username: "BillyBob", password: "321", isEmployed: false, RoleId: adminRole.id, Email: emailAddresses[1]});
                            const user3 = await User.create({username: "SillySally", password: "1234", isEmployed: true, RoleId: userRole.id, Email: emailAddresses[2]});
                            const user4 = await User.create({username: "SillySally1", password: "1234", isEmployed: true, RoleId: userRole.id, Email: emailAddresses[3]});
                            const user5 = await User.create({username: "SillySally2", password: "12345", isEmployed: false, RoleId: userRole.id, Email: emailAddresses[4]});
                            const user6 = await User.create({username: "SillySally3", password: "123456", isEmployed: true, RoleId: userRole.id, Email: emailAddresses[5]});
                            const user7 = await User.create({username: "SillySally4", password: "12345", isEmployed: false, RoleId: userRole.id, Email: emailAddresses[6]});
                            const user8 = await User.create({username: "SillySally5", password: "123456", isEmployed: false, RoleId: userRole.id, Email: emailAddresses[7]});
                            const user9 = await User.create({username: "SillySally6", password: "1234567", isEmployed: false, RoleId: userRole.id, Email: emailAddresses[8]});
                            const user10 = await User.create({username: "SillySally7", password: "12345678", isEmployed: true, RoleId: userRole.id, Email: emailAddresses[9]});
