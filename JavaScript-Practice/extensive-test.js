...
// create a library
let library1 = new Library("Extensive Library");
...
testResults.push(library1.getBooks().length === 3); // true
// Test 2: Remove a book from the library
librar1.removeBook(book1);
testResults.push(library1.getBooks().length === 2); // true
// Test 3: Check out a book
library1.checkOutBook(book2);
testResults.push(library1.findBookByTitle('1984').isCheckedOut === true); // true
// Test 4: Check in a book
library1.checkInBook(book2);
testResults.push(library1.findBookByTitle('1984').isCheckedOut === false); // true
// Test 5: Find a book by title
testResults.push(library1.findBookByTitle('1984') === book2); // true
// add additional books to the library
library1.addBook(book4);
library1.addBook(book5);
library1.addBook(book6);
// Test 6: Find books by author
testResults.push(library1.findBooksByAuthor('J.D. Salinger')[0] === book6); // true
// Test 7: Get all checked out books (none at this point)
testResults.push(library1.getAllCheckedOutBooks().length === 0); // true
// Test 8: Get all overdue books (none at this point)
testResults.push(library1.getAllOverdueBooks().length === 0); // true
...
library1.updateBook(book2.title, newBookData);
testResults.push(library1.findBookByTitle('Nineteen Eighty-Four').author === 'Eric Arthur Blair'); // true
// Test 11: Find the most popular author
testResults.push(library1.findMostPopularAuthor() === 'Aldous Huxley'); // true
// Test 12: Compare two books
testResults.push(library1.compareTwoBooks(book1, book2) === book1); // true
...
library1.addMultipleBooks(newBooks);
testResults.push(library1.getBooks().length === 8); // true
library1.saveToLocalStorage();
